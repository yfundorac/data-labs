...
def swap_dictionary(value):
    new_dict = dict([(value, key) for key, value in numbers.items()])
    return new_dict[value]
    

def calculator(a, b, o):
    result = numbers[a] + numbers[b] if o == 'plus' else numbers[a] - numbers[b]
    return swap_dictionary(result)
        

if (a not in numbers.keys() or c not in numbers.keys()) or (b != 'plus' and b != 'minus'):
    print("I am not able to answer this question. Check your input.")
else:
    print(calculator(a, c, b))
